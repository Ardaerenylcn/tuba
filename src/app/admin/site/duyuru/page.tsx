...
const DEFAULTS: AnnouncementBarConfig = {
...
export default function DuyuruPage() {
  const [form, setForm] = useState<AnnouncementBarConfig>(DEFAULTS);
  ...
  useEffect(() => {
    fetch("/api/v1/admin/site-content/announcement_bar")
      .then((r) => r.json())
      .then((d) => {
        if (d.success && d.data?.value) {
          setForm({ ...DEFAULTS, ...(d.data.value as Partial<AnnouncementBarConfig>) });
        }
      })
      .finally(() => setLoading(false));
  }, []);

  function set<K extends keyof AnnouncementBarConfig>(key: K, value: AnnouncementBarConfig[K]) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const res = await fetch("/api/v1/admin/site-content/announcement_bar", {
      method: "PUT",
  ...
  return (
    <form onSubmit={handleSave} className="p-6 max-w-2xl mx-auto flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-[var(--text-primary)]">Duyuru Çubuğu</h1>
          <p className="mt-0.5 text-xs text-[var(--text-muted)]">
            Sitenin üst kısmındaki ince duyuru bandını düzenleyin.
          </p>
        </div>
        <a
          href="/"
      ...
      <div className="border border-[var(--border)] bg-[var(--surface)] p-5 flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <label htmlFor="visible" className="text-sm text-[var(--text-primary)] cursor-pointer">
            Duyuruyu göster
          </label>
        </div>

        <Field label="Duyuru metni" hint="Kısa tutun — tek satırda görünecek">
          <input
            type="text"
            value={form.text}
            ...
            placeholder="Ücretsiz kargo tüm siparişlerde"
          />
        </Field>
      </div>
      ...
      {saved && (
        <p className="text-sm text-green-700">Değişiklikler kaydedildi. Sayfayı yenileyin.</p>
      )}
